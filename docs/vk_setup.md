...
- Для ЛС: ID пользователя.
- Для беседы: `peer_id`.

### 3. Pairing после setup
1. Setup helper запрашивает pair code из локального API.
2. Если в `VK_ALLOWED_PEERS` несколько ID, helper попросит `PAIRING_PEER_ID`.
...
- Если setup записал `SERVICE_MODE=fallback-local`, запускайте:
  - `vk-openclaw start`
  - `vk-openclaw status`
- Затем завершите pairing (`/pair <code>` в VK чате).
- Пока pairing не завершен, ответ `Pair this phone first with /pair <code>.` является ожидаемым.
...
- если видите `Unknown or blocked command`, сначала завершите pairing.
...
- используйте `fallback-local` через `vk-openclaw start/status/stop`.
...
3. If service mode is `fallback-local`, run `vk-openclaw start` and `vk-openclaw status`.
4. Complete pairing helper.
5. Validate in VK: `/status`, then `/ask hello`.
...
Fallback note:
- Before pairing is done, `Pair this phone first with /pair <code>.` is expected.
